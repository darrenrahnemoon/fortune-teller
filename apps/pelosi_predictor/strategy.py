...
from apps.pelosi_predictor.preprocessor.prediction import PelosiPredictorPrediction
from apps.pelosi_predictor.config import PelosiPredictorStrategyConfig

logger = Logger(__name__)

@dataclass
class PelosiPredictorStrategy(Strategy):
	config: PelosiPredictorStrategyConfig = None

	trainer_service: PelosiPredictorTrainerService = None
	tuner_service: PelosiPredictorTunerService = None
	predictor_service: PelosiPredictorPredictorService = None
	...
	def ensure_only_one_open_order_at_a_time(self, prediction: PelosiPredictorPrediction):
	...
	def ensure_tp_change_within_range(self, prediction: PelosiPredictorPrediction, tp_change: FloatRangeConfig):
	...
	def ensure_risk_over_reward_within_range(self, prediction: PelosiPredictorPrediction):
	...
	def ensure_spread_within_range(self, prediction: PelosiPredictorPrediction):
	...
	def block_invalid_sl_tp(self, prediction: PelosiPredictorPrediction):
	...
	def block_running_losses(self, prediction: PelosiPredictorPrediction, position: Position):
		if position.type != prediction.action:
			return
	...
	def close_position(self, position: Position, prediction: PelosiPredictorPrediction):
		position.close()
		logger.info(f'Closed Position due to trend change.\n{position}\n{prediction}')

	def modify_position(self, position: Position, prediction: PelosiPredictorPrediction):
		old_sl = position.sl
		old_tp = position.tp
	...
	def place_order(self, prediction: PelosiPredictorPrediction):
		order = Order(
