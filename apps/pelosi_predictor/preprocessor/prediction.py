...
@dataclass
class PelosiPredictorModelOutput:
	direction: int = None
	max_high_change: float = None
	min_low_change: float = None

@dataclass
class PelosiPredictorPrediction:
	strategy_config: PelosiPredictorStrategyConfig = field(repr = False)

	# Raw output
	model_output: PelosiPredictorModelOutput = None
